...
4. **Predictable** — Given the same inputs, the outputs are always the same. Features that introduce non-determinism (e.g. `incremental`, caches, package-lock drift) are disallowed.
...
- **Compositional** — Designed to work with fundamentals rather than fight them. You pick the preset that matches what you are building and extend it as needed.
