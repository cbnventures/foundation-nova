---
id: check-versions
title: Check System Versions
description: Display installed versions of common developer tools and environments.
tags:
  - versions
  - cli
  - system
...
The `version` command collects system information across developer tools, operating systems, browsers, and interpreters.

It outputs results in a consistent, easy-to-read format that can be copied directly into issues or documentation.

:::tip
The output is **Markdown-ready**. Copy and paste the tables directly into GitHub issues or documentation without extra formatting.
:::
...
<Tabs>
  ...
  <TabItem value="nova" label="nova (installed)">
...
<Tabs>
  <TabItem value="raw-node" label="Raw (actual output)" default>
...
| Code | Meaning | Result                                              |
|------|---------|-----------------------------------------------------|
| `0`  | Success | At least one category was checked and results shown |
...
- **Browser detection** — Certain browsers (e.g., LibreWolf) are platform-specific or may not be supported. Refer to [the table](/docs/cli/utilities/check-versions#web-browsers) above to confirm availability.
